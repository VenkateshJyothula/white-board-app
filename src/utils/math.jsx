...
export const isPointCloseToEllipse=(x1,y1,x2,y2,pointX,pointY)=>{
  const {x,y}=midPointBtw({x:x2,y:y2},{x:x1,y:y1});
  const a=((x2-x1)/2),b=((y2-y1)/2);
  const val=Math.pow((pointX-x)/a,2)+Math.pow((pointY-y)/b,2);
  return Math.abs(val-1)<0.1;
}
